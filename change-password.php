...
$udate=date('d-m-Y h:i:s', time());;
	$sql="SELECT password FROM homeowner where password=?";
	...
	if($row_cnt>0)
	{
		$con="update home_owner set password=?,updationdate=?  where password=?";
$chngpwd1 = $mysqli->prepare($con);
$chngpwd1->bind_param('sss',$np,$udate,$ai);
  $chngpwd1->execute();
...
<?php $result ="SELECT updationdate FROM home_owner WHERE password=?";
$stmt = $mysqli->prepare($result);
$stmt->bind_param('s',$ai);
$stmt->execute();
